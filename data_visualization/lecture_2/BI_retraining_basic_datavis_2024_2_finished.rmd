...
hogwarts |> 
  ...
  geom_point(aes(x = `Charms exam`, 
                 y = `Transfiguration exam`), shape = 21, 
...
```{r}
hogwarts |> mutate(wandCore = as.factor(wandCore)) |> 
  ggplot()+
...
```{r}
ggarrange(plotlist = list(scatterStroke, scatterShapeCourse), nrow = 2,
          #ncol = 2, 
          heights = c(1,2))#, widths = c(1,2))
```
...
#можно еще подписи немного поворачивать под углом: theme(axis.text.x = element_text(angle = 20))
#scale_x_continuous(breaks = seq(0,7000,1000), labels = paste0(seq(0,7000,1000), " p.")) так ось x будет подписана от 0 до 7000 (или сколько влезет), с шагом в 1000, и к каждому шагу будет добавлно p. (points).
#theme(legend.position = "bottom") так меняем расположение легенды, так вообще под график
#theme(legend.position = "right", legend.justification = "bottom") - так справа внизу графика
...
# или scale_x_continuous(breaks = seq(0,7000,1000), labels = paste0(seq(0,7000,1000), " p."), limits = c(0, 7000))
```
