...
roc_curve_1 %>% coords(x = "best", best.method = "closest.topleft") #Здесь можно вместо topleft использовать критерий Юдена
...
```{r}
#например, мы можем сделать модель, в которую запихнем сразу несколько признаков, потенциально неплохо предсказывающих окклюзию, и построить рок-кривую для этой модели (и она, вероятно, будет лучше предсказыввать, чем признаки по отдельности)
#Или воспользоваться нашими знаниями из области физиологии. В доменной области можем попытать врача, а что-нибудь еще вычисляется из этих данных.
arteries_data %>%
    mutate(Perf_Cv = (Perf_SD / Perf_M * 100) %>% round(1)) %>% #тут что-то не так, данных не хватает. Оценка перфузии, индекс перфузии.
...
```{r}
#Индекс сосудистой проводимости

arteries_data_cleaned %>%
  mutate(MAP = DAP + 1/3*(SAP - DAP),    #считаем среднее артериальное давление
         CVC = (Perf_M/MAP) %>% round(2)) %>%        #индекс - это перфузия, деленная на среднее артериальное давление  
  roc(Arterial_occlusion ~ CVC, data = ., ci = T) -> roc_curve_4

roc_curve_4 #Этот показатель обладает некоторым преимуществом и по сравнению с просто перфузией, и по сравнению с систолическим и диастолическим артериальным давлением. Применяем физиологически подход и известные расчетные индексы из этой области
#А возраст пациента работает не хуже, и посмотреть его намного легче
#В логистической регрессии можем посмотреть, как независимо от возраста предикторы работают, связаны они или нет.
```
